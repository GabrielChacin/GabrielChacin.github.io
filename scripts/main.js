...
let canConnectArduino = false;

const arduinoButton = document.getElementById('connect-arduino');
arduinoButton.onclick = () => {
    if (canConnectArduino) {
        return;
    }
    console.log(Arduino)
    Arduino.start().then(() => {
        console.log('Arduino conectado.');
        canConnectArduino = true;
    }).catch((error) => {
...
window.addEventListener('load', () => {
    ...
    window.applyFilters = applyFilters; // Exportar la función para que pueda ser llamada desde otro archivo js
    ...
    function updateTsunamiMarkers(epochNumber, minMagnitude) {
            ...
            try {
                ...
                marker.on('click', function() {
                    ...
                    if (earthquakeMagnitude < 6) {
                        tsunamiSound.volume = 0.1; // Volumen bajo para magnitudes menores a 6
                    } else if (earthquakeMagnitude >= 6 && earthquakeMagnitude < 7) {
                        tsunamiSound.volume = 0.2; // Volumen medio para magnitudes entre 6 y 7
                    } else if (earthquakeMagnitude >= 7 && earthquakeMagnitude < 8) {
                        tsunamiSound.volume = 0.4; // Volumen alto para magnitudes entre 7 y 8
                    } else {
                        tsunamiSound.volume = 1.0; // Volumen máximo para magnitudes mayores a 8
                    }

                    tsunamiSound.play(); // Reproducir el sonido
                    controlArduino(earthquakeMagnitude);
        ...
        window.updateTsunamiMarkers = updateTsunamiMarkers;
    ...
    // Función para controlar el Arduino
    function controlArduino(magnitude) {
        if (!canConnectArduino || (!magnitude && magnitude !== 0)) {
            return;
        }
        if (magnitude < 6) {
            // Gira a la derecha y vuelve al centro
            Arduino.servoWrite({ pin: 6, value: -580 }); // Derecha
            setTimeout(() => {
                Arduino.servoWrite({ pin: 6, value: -190 }); // Centro
            }, 1000);
        } else if (magnitude >= 6) {
            // Gira a la derecha y luego a la izquierda
            Arduino.servoWrite({ pin: 6, value: -580 }); // Derecha
            setTimeout(() => {
                Arduino.servoWrite({ pin: 6, value: 140 }); // Izquierda
                setTimeout(() => {
                    Arduino.servoWrite({ pin: 6, value: -190 }); // Centro
                }, 1000);
            }, 1000);
